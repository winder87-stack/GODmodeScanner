...
# ============================================================================
# Pump.fun Specific Risk Aggregation Method
# ============================================================================
    ...
    def aggregate_risk(
        ...
        - graduation_rate: Percentage of picks that hit Raydium (0-100)
        - curve_entry: Position on bonding curve at entry (0-100%, lower = earlier)
        - consistency: Number of trades (sample size indicator)
        - profit_total: Total SOL profit
        - avg_hold_time: Average hold time in seconds

        Mapping to Risk Factors:
        - behavior: Derived from win_rate, curve_entry, consistency
        - timing: Derived from curve_entry (early entry = higher risk)
        - network: Default 0.5 (requires graph analysis)
        - volume: Derived from profit_total and consistency

        Args:
            win_rate: Win rate percentage (0-100)
            graduation_rate: Raydium graduation rate (0-100)
            curve_entry: Curve position at entry (0-100, lower = earlier)
            consistency: Number of trades
            profit_total: Total SOL profit
            avg_hold_time: Average hold time in seconds
            wallet_address: Wallet address for logging
        ...
        Returns:
            PumpFunRiskScore object with all required fields

        Example:
            >>> score = aggregator.aggregate_risk(
            ...     win_rate=85.0,
            ...     graduation_rate=70.0,
            ...     curve_entry=3.5,  # Early entry
            ...     consistency=25,
            ...     profit_total=150.5,
            ...     avg_hold_time=300.0
            ... )
            ...
            0.82
            >>> print(score.severity.value)
            critical
        """
        # Convert pump.fun metrics to risk factors
        # Handle edge cases first
        if consistency < 5:
            # Low sample size - use conservative estimates
            self.logger.warning(
                "low_sample_size",
                wallet=wallet_address,
                trades=consistency,
                warning="Using conservative estimates"
            )
            # Add small penalty for low sample size
            win_rate = max(0, win_rate - 10)
            graduation_rate = max(0, graduation_rate - 10)

        # Hard filters (reject low-quality wallets before scoring)
        # Filter 1: Losers (win_rate < 45%)
        is_loser = win_rate < 45.0
        
        # Filter 2: MEV bots (hold_time < 10s = instant flip)
        is_mev_bot = avg_hold_time < 10.0

        # Map pump.fun metrics to risk factors (0-1 scale)
        
        # behavior: Aggressive trading patterns
        # - High win_rate + early curve_entry = aggressive/sniper behavior
        # - Normalize to 0-1
        behavior_factor = (win_rate / 100.0) * 0.6 + (1.0 - min(curve_entry / 100.0, 1.0)) * 0.4
        
        # timing: Early entry timing
        # - Lower curve_entry percentage = earlier entry = higher timing risk
        # - 0% entry = 1.0 (highest risk), 100% entry = 0.0 (lowest risk)
        timing_factor = max(0.0, 1.0 - (curve_entry / 100.0))
        
        # network: Insider network connections
        # - Requires graph analysis, default to 0.5
        network_factor = 0.5
        
        # volume: Whale-like volume
        # - Higher profit_total and consistency = whale-like
        ...
        if is_mev_bot:
            self.logger.info(
                "wallet_flagged_as_mev_bot",
                wallet=wallet_address,
                avg_hold_time=avg_hold_time
            )
            behavior_factor = max(0.0, behavior_factor - 0.3)

        # Calculate composite risk score using standard risk factors
        risk_score = self.aggregate_risk_factors(
            behavior=behavior_factor,
            timing=timing_factor,
            network=network_factor,
        ...
        # Calculate King Maker status
        # King Maker: High graduation_rate + early entry
        is_king_maker = (
            graduation_rate >= 60.0 and  # High graduation rate
            curve_entry <= 10.0 and  # Early curve entry
            consistency >= 10 and  # Minimum sample size
            not is_loser and
            not is_mev_bot
        )

        # Apply King Maker multiplier
        if is_king_maker:
            # King Makers get bonus to their score
            score = risk_score.composite_score
            # Boost by 10-20% for King Makers
        ...
        # Return pump.fun specific risk score
        return PumpFunRiskScore(
            composite_score=boosted_score,
            confidence=risk_score.confidence,
            confidence_interval=risk_score.confidence_interval,
...
class PumpFunRiskScore:
    ...
    Attributes:
        composite_score: Final risk score (0-1)
        confidence: Bayesian confidence (0-1)
        confidence_interval: 95% credible interval
        severity: Risk severity level
        is_king_maker: Whether wallet qualifies as King Maker
        behavior_factor: Aggressiveness score
        timing_factor: Early entry timing score
        network_factor: Insider network score
        volume_factor: Whale volume score
        win_rate: Raw win rate percentage
        graduation_rate: Raydium graduation rate
        curve_entry: Average curve position
        consistency: Number of trades
        profit_total: Total SOL profit
        avg_hold_time: Average hold time in seconds
        is_loser: Flagged as loser
        is_mev_bot: Flagged as MEV bot
