...
class AggressiveSolanaClient:
    ...
    def __init__(
        ...
        # Load endpoints from environment variable or use defaults
        if rpc_endpoints:
            self.rpc_endpoints = rpc_endpoints
        elif os.getenv("SOLANA_RPC_ENDPOINTS"):
            # Load from environment variable (comma-separated)
            env_endpoints = os.getenv("SOLANA_RPC_ENDPOINTS", "")
            self.rpc_endpoints = [ep.strip() for ep in env_endpoints.split(",") if ep.strip()]
        else:
            # Default endpoints
            self.rpc_endpoints = [
        ...
        self.total_429s = 0
        self.endpoint_cooldowns = {}  # Track per-endpoint cooldowns
        ...
        logger.info(f"🚀 AggressiveSolanaClient initialized")
    ...
    def _get_next_endpoint(self) -> str:
        """Get the next endpoint in round-robin fashion, skipping endpoints in cooldown."""
        now = time.time()
        ...
        for _ in range(len(self.rpc_endpoints)):
            ...
            if endpoint in self.endpoint_cooldowns:
                cooldown_until = self.endpoint_cooldowns[endpoint]
                if now < cooldown_until:
                    # Endpoint is in cooldown, skip it
                    remaining = cooldown_until - now
                    logger.debug(f"⏭️  Skipping {endpoint[:30]}... (cooldown: {remaining:.1f}s remaining)")
                    self.current_endpoint_index = (self.current_endpoint_index + 1) % len(self.rpc_endpoints)
                    continue

            # Endpoint is available, return it
            self.current_endpoint_index = (self.current_endpoint_index + 1) % len(self.rpc_endpoints)
            return endpoint

        # All endpoints in cooldown, reset and return first
        self.current_endpoint_index = (self.current_endpoint_index + 1) % len(self.rpc_endpoints)
        return self.rpc_endpoints[original_index]
    ...
    async def _make_request(
        ...
        try:
            ...
            if retry_count < self.max_retries:
                ...
                logger.warning(
                    f"⚠️  Request failed (attempt {retry_count + 1}/{self.max_retries}): {e}"
   ...
   Total 429s: {self.total_429s}"""
            ...
            if self.consecutive_successes >= self.growth_threshold:
                ...
                logger.info(
                    f"✅ SUCCESS: Increasing speed to {self.current_rps:.2f} RPS "
                    f"(was {old_rps:.2f})"
                )
            
            elif self.consecutive_successes % 25 == 0:
                logger.info(
                    f"📈 Progress: {self.consecutive_successes}/{self.growth_threshold} "
    ...
    async def request(
        ...
        """
        Execute a single RPC request with endpoint rotation on 429/403 errors.
        ...
        # Build payload (reused for retries)
        payload = {
            "jsonrpc": "2.0",
        ...
        # Endpoint rotation loop - try all endpoints on 429/403
        max_endpoint_attempts = len(self.rpc_endpoints)

        for endpoint_attempt in range(max_endpoint_attempts):
            # Get next available endpoint (skips those in cooldown)
            url = self._get_next_endpoint()
            user_agent = self._get_random_user_agent()

            logger.debug(f"Trying endpoint: {url[:40]}... (attempt {endpoint_attempt + 1}/{max_endpoint_attempts})")
            ...
            try:
                ...
                # Process feedback (updates cooldowns, RPS, etc.)
                self._process_feedback(response)

                # Check if we got a retryable error
                if response.status_code in (429, 403):
                    logger.warning(
                        f"{response.status_code} on {url[:30]}... - rotating to next endpoint"
                    )
                    continue  # Rotate to next endpoint
                ...
                logger.warning(f"HTTP error {e.response.status_code} on {url[:30]}... - rotating")
                # Add to cooldown if rate limited
                if e.response.status_code == 429:
                    self.endpoint_cooldowns[url] = time.time() + 60
                continue  # Rotate to next endpoint

            except httpx.RequestError as e:
                logger.warning(f"Request error on {url[:30]}...: {e} - rotating")
                continue  # Rotate to next endpoint

        # All endpoints failed
        logger.error(f"All {max_endpoint_attempts} endpoints failed for method: {method}")
        raise httpx.HTTPStatusError(
            "All RPC endpoints failed",
            request=None,
            response=httpx.Response(503, request=None)
        )
    async def batch(
        self,
        ...
        if not batch:
            logger.debug(f"📦 All {len(method_params_list)} requests cached")
        ...
        logger.debug(f"📦 Batch: {len(batch)} requests, {len(cache_results)} cached")
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "endpoints_in_cooldown": len([e for e, t in self.endpoint_cooldowns.items() if t > now]),
            "cooldowns": {ep[:30]+'...': f"{t-now:.1f}s" for ep, t in self.endpoint_cooldowns.items() if t > now},
...
async def demo_aggressive_client():
    ...
    try:
        ...
        logger.info("\n🔹 Testing single requests with caching...")
        for wallet in test_wallets:
            result = await client.request(
                "getAccountInfo",
                [wallet, {"encoding": "base64"}]
            )
            logger.info(f"✅ Request for {wallet[:8]}... completed")
        ...
        logger.info("✅ Request completed (likely from cache)")
        ...
        logger.info(f"✅ Batch of {len(results)} requests completed")
